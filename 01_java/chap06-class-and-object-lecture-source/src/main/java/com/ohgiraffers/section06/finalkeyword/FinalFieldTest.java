...
public class FinalFieldTest {
    ...
    /*
    * final
    * : final은 변경 불가의 의미를 담고 있는 키ㅜ어드
    *
    * final이 사용되는 곳
    * 1. 지역변수 : 초기화 이후에 값 변경 불가
    * 2. 매개변수 : 메소드 호출 시 전달한 인자 변경 불가
    * 3. 전역변수 : 인스턴스 생성 후 , 초기화 이후 값 변경 불가
    * 4. 클래스(static) 변수 : 프로글램 start 후 값 변경 불가
    * 5. non-static 변수: 메소드 재작서(overriding) 불가
    ...
    * */
    ...
    // private final int nonStaticNum = 0;
    ...
    * 하지만 static은 프로그램이 start 될 때 할당하기 때문에 그 전에는 초기화가 일어나지 못한다.
    * 초기화가 되지 않은 상태로 선언된 것과 동일하여
    * 기본값으로 초기화 된 후 값을 변경하지 못하면 에러가 발생한다.
    * */
