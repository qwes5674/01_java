...
public class Car {
    ...
    * */
    ...
    public void startUp() {
        ...
        if (isOn) {
            System.out.print("이미 시동이 걸려있습니다.");
        } else {
            this.isOn = true;
            System.out.print("시동을 걸었습니다. 이제 출발할 준비가 되었습니다.");
        }
    }
     ...
     //시동이 걸린 상태일 경우, 자동차는 시속이 10km/h 증가하며 앞으로 나간다.
        public void go() {
         if(isOn){
             System.out.println("차가 앞으로 움직입니다. 속도가 10km증가합니다.");
             this.speed += 10;
         } else{
    ...
    // 시동이 걸려있고, 달리는 상태일 때 멈출 수 있다.
    // 달리는 중인 경우 브레이크를 밟으면 자동차의 시속은 0으로 떨어지며 멈춘다.
    public void stop(){
        if(isOn){
            if(this.speed > 0){
                this.speed = 0;
                System.out.println("브레이크를 밟았습니다. 차는 멈춤니다.");
            } else {
