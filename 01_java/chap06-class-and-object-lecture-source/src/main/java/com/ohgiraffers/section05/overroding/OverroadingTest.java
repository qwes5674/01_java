package com.ohgiraffers.section05.overroding;

public class OverroadingTest {

    // 오버로딩
    // 동일한 이름의 생성자나 메소드를 여러개 작성 할 수 있게
    // -> 같은 이름의 메소드여도 매개변수 부분의 타입, 개수,
    // 순서를 다르게 작성하면 서로 다른 메소드나 생성자로 인식
    // -> 같은 이름으로 여러 메소드를 만드는 것을 오버로딩이라고 한다.

    /*
    * 오버로딩의 사용 이유
    *
    * 매게변수의 종류별로 메소드 내용을 다르게 작성해야 하는 경우가 종종 있다.
    ...
    * 메소드의 시그니처(Signature)가 다르면 다른 메소드로 인식하기 떄문이다.
    * 즉, 시그니쳐 중 메소드 이름은 동일해야 하기 떄문에
    * 파라미터 선언부가 다르게 작성되어야 오버로딩이 석립됨.
    * */
    ...
    * 메소드의 시그니쳐가 달라야 하기 떄문에 접근제한자나 반환형은 오버로딩 석립 요건에 해당하지 않음.
    * */
    ...
    //매개변수 개수에 따른 오버로딩 성립 확인
    public void test(int num1, int num2){} //매개변수의 개수에 따라 오버로딩 성립

    // 매개변수 변수명에 따른 오버로딩 성립 확인
    //public void test(int num2){} //매개변수 변수명은 메소드 시그니쳐에 해당하지 않음.

    // 매개변수 타입 변화에 따른 오버로딩 확인
    public void test (int num, String str){} // 매개변수의 탕입에 따라 오버로딩 성립
