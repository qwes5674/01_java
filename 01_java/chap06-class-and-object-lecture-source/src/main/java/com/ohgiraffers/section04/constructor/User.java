...
public class User {
    ...
    * */
    ...
    /*
    * 생성자 작성 시 주의할 점
    * 1. 생성자 메소드는 반드시 클래스의 이름과 동일해야 한다. (대/소문자까지_
    ...
    * 2. 매개변수 있는 생성자
    *
    * 매개변수가 있는 생성자가 한개라ㅏ도 존재하면 기본생성자를 자바 compiler가 자동으로 추가하지 않는다.
    ...
    * */
    ...
    public User(String id, String pwd, String name){
        // 매개변수 있는 생성자의 주 목적 -> 인스턴스 생성 시점에 필드를 원하는 값으로 초기화하기 위함
        this.id = id;
        this.pwd = pwd;
        this.name = name;
 ...
 //       this.enrollDate = enrollDate;
        ...
        /*
         * this() 사용하기
         * this() 는 통일 클래스 내에 작성한 다른 생성자 메소드를 호출하는 구문읻.
         * 괄호 안에 매개변수의 타입, 개수, 순서에 맞는 생성자를 호출하고 복귀한다. (메소드 동일)
         * this ()는 가장 첫 줄에 선언해야 하며, 그렇지 않은 경우 Compile Error각 발생한다.
         * */

        this(id, pwd, name); // 미리 작성한 3개의 필드를 초기화하는 생성자로 매개변수로 받은 값을 전다.
