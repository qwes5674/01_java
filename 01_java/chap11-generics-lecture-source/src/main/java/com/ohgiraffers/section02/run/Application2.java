...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        *                      Type과 Type의 부모를 이용해 생성한 인스턴스만 인자로 입력
        * */
        ...
        System.out.println("================== <?> ===============");
        //RabbitFarm을 생성만 가능하면 <?>에서는 모두 매개변수를 사용 가능하다.
        wildCardFarm.anyType(new RabbitFarm<Rabbit>(new Rabbit()));
        wildCardFarm.anyType(new RabbitFarm<Bunny>(new Bunny()));
        wildCardFarm.anyType(new RabbitFarm<DrunkenBunny>(new DrunkenBunny()));

        System.out.println("================== <? extends Bunny> ===============");
        //extend Bunny이기 떄문에 Bunny이거나 Bunny의 후손 토끼 농장만 매개변수로 사용 가능
        // 상위 타입으로 만든 토끼 농장은 매개변수로 사용 불가능
        //wildCardFarm.extendsType(new RabbitFarm<>(new Rabbit()));
        wildCardFarm.extendsType(new RabbitFarm<Bunny>(new Bunny()));
        wildCardFarm.extendsType(new RabbitFarm<DrunkenBunny>(new DrunkenBunny()));

        System.out.println("================== <? super Bunny> ===============");
        // extens Bunny의 <? super> 이기 때문에 Bunny이거나 Bunny상위 토끼농장만 매개변수로 사용가능
        // 후손타입으로 만든 토끼 매개변수 사용가능
