...
public class Application {
    ...
    public static void main(String[] args) {
      ...
      * 부모가 가지고 있는 재산(자바에서는 클래스가 가지는 멤버)를 자식이 물려받는 다는 읨미.
      * 클래스 또한 부모클래스와 자식클래스로 역할을 나누어서 부모가 가지는 멤버를 자식이
      * 물려받아 자기의 멤버인 것 처럼 사용 할 수 있도록 만든 기술
      *
      * 단순히 물려받는 개념보다, 자바에서 상속은 부모클래스의 확장(extend)의 개념을 가진다.
      * 물려받아서 자신의 것 처럼 사용 하는 것 뿐 아니라, 추가적인 멤버도 작성이 가능하다.
      * 추가로, 메소드 재정의(overriding)을 이용해서 무모가 가진 메서드를 재정의 하는 것도
      * 가능하다.
      * */
       ...
       * */
        ...
        *   1-2. 클래스간의 계층 관계가 형성되며 다향성의 문법적인 토대가 된다.
        * 2. 단점
        *   2-1. 부모클래스의 기능을 추가/변경할 시 자식클래스가 정상적으로 동작하는지에 대한 예측이 힘들다.
        *   2-2. 부모클래스의 변경이 쉽지 않다. 자식클래스에서 중요하게 사용하는 기능인 경우
        *        부모클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다.
        *   2-3. 부모클래스에서 의미있던 기능이, 자식클래스에서는 무의미 할 수 있다.
        * */

        /*
        * 장점 단점 고려했을때
        * 상속을 사용하는 떄가 언젠지 정해서 잘 사용을 해야한다.
        ...
        * 적절 해야한다. 적절한 책임을 가진 객체들이 서로 협력(메시지 수신과 응답)을 통해 프로그램을
        * 동작하는 것 => 객체지향 프로그램
        * */
        ...
        //firCar.runningStatus; //private 필드에 접근 불가
        //fireCar is running(); //private 메소드는 접근불가 -> protect로 변경 후 사용 가능
